      ...
      <section className="organization pb-3">
        ...
        <div className="pt-5 pb-5">
          <div className="mb-4 text-center">
            <Text variant="xLargePlus" styles={semibold}>
              {locale?.aboutUs.header1}
            </Text>
          </div>
              ...
              <Persona
                ...
                onRenderPrimaryText={() => (
                  <Text variant="medium" styles={semibold}>
                    {networkMembers[0].name}
                  </Text>
                )}
                secondaryText={networkMembers[0].username}
                onRenderSecondaryText={() => (
                  <Text variant="medium">
                    <Link href={`https://t.me/${networkMembers[0].username}`}>@{networkMembers[0].username}</Link>
                  </Text>
                )}
              />
            )}
                    ...
                    <Col className="mb-3" lg={3} md={6} sm={6} xs={12} key={i}>
                      <div style={{ maxWidth: 240, marginLeft: "auto", marginRight: "auto" }}>
                        <Persona
                          ...
                          onRenderSecondaryText={() => (
                            <Text variant="medium">
                              <Link href={`https://t.me/${x.username}`}>@{x.username}</Link>
                            </Text>
                          )}
                          tertiaryText={x.delega}
                          onRenderTertiaryText={() => (
                            <TooltipHost content={x.delega}>
                              <Text variant="small">{x.delega}</Text>
                            </TooltipHost>
                          )}
                        />
                      </div>
        ...
        <div className="pb-5">
          <Container>
            <div className="mb-4 text-center">
              <Text variant="xLargePlus" styles={semibold}>
                {locale?.contributors.header1}
              </Text>
            </div>

            <Row className="justify-content-center" style={{ rowGap: 15 }}>
              {domLoaded &&
                developers.map((x: Developer, i: number) => (
                  <>
                    <Col className="mb-3" lg={4} sm={6} xs={12} key={i}>
                      <div style={{ maxWidth: 250, marginLeft: "auto", marginRight: "auto" }}>
                        <Persona
                          ...
                          onRenderPrimaryText={() => (
                            <Text variant="medium" styles={semibold}>
                              <Link href={x.github}>{x.name}</Link>
                            </Text>
                          )}
                          secondaryText={x.username}
                          onRenderSecondaryText={() => (
                          ...
                          onRenderTertiaryText={() => (
                            <TooltipHost content={x.description[language!]}>
        ...
        <div className="pt-5 pb-5 text-center">
          <Container className="flex-column d-flex">
            <Text styles={bold} variant="superLarge">
              {locale?.aboutUs.contact.title}
            </Text>
            <Text className="mb-3">{locale?.aboutUs.contact.description}</Text>

            <div style={{ margin: "0 auto", maxWidth: 600 }}>
              <DefaultButton text={locale?.aboutUs.contact.button} iconProps={{ iconName: "AiOutlineMail" }} href="mailto:info@studentiunimi.it" />
            </div>
          </Container>
        </div>
      </section>
