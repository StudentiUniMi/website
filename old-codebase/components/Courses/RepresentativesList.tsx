              ...
              props.representatives.map((x, i) => (
                <Col key={i} xl={3} lg={3} md={4} sm={6} xs={12} className="mb-3 col-persona">
                  {(() => {
                    ...
                    if (x.tguser?.username !== "")
                      primaryText = (
                        <>
                          <Icon iconName="IoIosMail" style={{ color: theme.palette.themePrimary }} />
                          &nbsp;<Link href={`mailto:${x.tguser?.username}`}>{`${x.tguser?.first_name ?? ""} ${x.tguser?.last_name ?? ""}`}</Link>
                        </>
                      )
                    else {
                      primaryText = `${x.tguser?.first_name ?? ""} ${x.tguser?.last_name ?? ""}`
                    }
                    return (
                      <Persona
                        imageUrl={imageUrl}
                        onRenderPrimaryText={() => primaryText}
                        text={`${x.tguser?.first_name ?? ""} ${x.tguser?.last_name ?? ""}`}
                        secondaryText={x.degree_name}
